...
function initEvent() {
    ...
    createEvent.variables = {                                            
        title: "",
        description: "",
        beginsOn: moment().toISOString(),
        endsOn: moment().add(2, 'weeks').toISOString(),
        status: 'CONFIRMED',
        category: 'MEETING',
        visibility: 'PUBLIC',
        joinOptions: 'FREE',
...
async function postToMobilizon(eventvars) {
   /* console.log(eventvars.location)
    ...
    try {
        ...
        debug(".json decode error? %s", error);
    ...
    try {
        ...
        debug("Error in createEvent =( %O", error);
        debug(JSON.stringify(responseBody, undefined, 2));
        throw error;
    }
    ...
    if(!_.isInteger(id)) {
        ...
        throw new Error("not found an event Id");
    }
    ...
    debug("Successful event created: %j", result);
    console.log(`Event created at: ${responseBody.data.createEvent.url}`);
    ...
    return responseBody.data.createEvent;
}
